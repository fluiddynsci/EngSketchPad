			      ...
			      int ietf_version_or_minus_one)
{
	...
	while (ext && ext->callback && !handled) {
                ...
                m = ext->callback(context, ext, wsi,
                                  ...
                                  (void *)(long long)n, (void *)address, port);
#else
		m = ext->callback(context, ext, wsi,
			          LWS_EXT_CALLBACK_CAN_PROXY_CLIENT_CONNECTION,
                              ...
                              void *userdata)
{
	struct libwebsocket *ws =
		libwebsocket_client_connect(context, address, port,
                                            ssl_connection, path, host, origin,
                                            protocol, ietf_version_or_minus_one);

	if (ws && !ws->user_space && userdata) ws->user_space = userdata;
