        ...
        void  *data)                    /* (in)  project[:branch[.revision]][*] */
{
    ...
    int            buildTo, builtTo, nchange, *changes, nhist, nline, ivalue, *tempInt, nrow, ncol;
    const int      *partial;
    short          datetime[6];
...
#define MAX_BUFFER_LEN  10*MAX_FILENAME_LEN
    ...
    /* check the state of the capsMode->parPhase */
        ...
        /* capsMode->parPhase is open, so start CAPS in continuation mode */
            ...
            /* inform CAPS of latest intent phrase */
            status = caps_intentPhrase(capsMode->projObj, 1, (const char **)(lines[0]));
            CHECK_STATUS(caps_intentPhrase);

            /* make a new __intent__ parameter */
            status = caps_childByName(capsMode->projObj, VALUE, PARAMETER, "__intent__",
                                      &intentObj, &nerror, &errors);
            if (status != SUCCESS || nerror > 0) {
                ivalue = 1;
                status = caps_makeValue(capsMode->projObj, "__intent__", PARAMETER, 1,
                                        1, 1, &ivalue, NULL, NULL, &intentObj);
                CHECK_STATUS(caps_makeValue);

            /* or increment the existing __intent__ */
            } else {
                status = caps_getValue(intentObj, &vtype, &nrow, &ncol, &myData, &partial, &units, &nerror, &errors);
                (void) caps_printErrors(0, nerror, errors);
                CHECK_STATUS(caps_getValue);

                tempInt = (int *)myData;
                ivalue = tempInt[0] + 1;
                status = caps_setValue(intentObj, Integer, 1, 1, &ivalue, NULL, NULL, &nerror, &errors);
                (void) caps_printErrors(0, nerror, errors);
                CHECK_STATUS(caps_setValue);
            }
        ...
        char  command[])                /* (in)  command */
{
    ...
    int            ivalue, *tempInt, nerror, irev, count;
    int            nline, nanal, ianal, nbound, ibound, nhist, ihist, nhist2, ihist2;
    int            i, ibrch, nrow, irow, ncol, icol, max_resp_len=0, response_len=0, itype;
    ...
    char           *pEnd, begend[4], *name2, prefix, *vname, *aname;
    const char     *units;
    const void     *data;
...
//$$$            break;
    ...
    /* "getCvals|" */
        ...
        for (icval = 1; icval <= ncval; icval++) {
            ...
            if (strncmp(name2, "__", 2) == 0) continue;
    ...
    /* "intent|message|" */
        ...
        /* inform CAPS of latest intent phrase */
        status = caps_intentPhrase(capsMode->projObj, 1, (const char **)(&arg2));
        CHECK_STATUS(caps_intentPhrase);
        ...
        /* or increment the existing __intent__ */
            ...
            status = caps_getValue(intentObj, &vtype, &nrow, &ncol, &data, &partial, &units, &nerror, &errors);
            (void) caps_printErrors(0, nerror, errors);
            CHECK_STATUS(caps_getValue);
