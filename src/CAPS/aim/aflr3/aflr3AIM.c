...
#if 0
static int get_MeshIndex(const mapAttrToIndexStruct *meshMap, ego ebody,
                    ...
                    /*@unused@*/ int restart, capsValue *aimInputs)
{
    ...
    int i, j, ibody, iglobal, nglobal, noDataTransfer=(int)false;
    ...
    if (aimInputs[Multiple_Mesh-1].vals.integer == (int) true) {
        ...
        for (ibody = 0; ibody < numSurfaceMesh; ibody++) {
            ...
            status = EG_loadTess(body, aimFile, &aflr3Instance->meshRef[0].maps[ibody].tess);
            AIM_STATUS(aimInfo, status);

            status = aim_newTess(aimInfo, aflr3Instance->meshRef[0].maps[ibody].tess);
            AIM_STATUS(aimInfo, status);

            // get the total number of points on the surface mesh
            status = EG_statusTessBody(aflr3Instance->meshRef[0].maps[ibody].tess, &body, &state, &nglobal);
            AIM_STATUS(aimInfo, status);

            // allocate the surface to volume mesh mapping
            AIM_ALLOC(aflr3Instance->meshRef[0].maps[ibody].map, nglobal, int, aimInfo, status);
            ...
            for (iface = 0; iface < nface; iface++) {
                // retrieve the surface to volume mapping
                snprintf(attrname, 128, "face_node_map_%d",iface+1);
                status = EG_attributeRet(aflr3Instance->meshRef[0].maps[ibody].tess, attrname, &atype,
                                         &nnode_face, &face_node_map, &preals, &pstring);
                AIM_STATUS(aimInfo, status);
                AIM_NOTNULL(face_node_map, aimInfo, status);
                ...
                for (i = 0; i < nnode_face; i++) {
                    // map from the face to the surface global indexing
                    status = EG_localToGlobal(aflr3Instance->meshRef[0].maps[ibody].tess, iface+1, i+1, &iglobal);
                    AIM_STATUS(aimInfo, status);

                    aflr3Instance->meshRef[0].maps[ibody].map[iglobal-1] = face_node_map[i];
                    if (face_node_map[i] == 0) noDataTransfer = (int)true;
                }

                // the mapping no longer needs to be stored in the tessellation
                status = EG_attributeDel(aflr3Instance->meshRef[0].maps[ibody].tess, attrname);
                AIM_STATUS(aimInfo, status);
            }
        }
    }
