                       ...
                       meshStruct *volumeMesh)
{
    ...
    for (bodyIndex = 0; bodyIndex < volumeMesh->numReferenceMesh; bodyIndex++) {
        ...
        status = EG_copyObject(body, NULL, &copy_body_tess[bodyIndex]);
        AIM_STATUS(aimInfo, status);

        status = EG_copyObject(volumeMesh->referenceMesh[bodyIndex].egadsTess, copy_body_tess[bodyIndex], &copy_body_tess[volumeMesh->numReferenceMesh+bodyIndex]);
        AIM_STATUS(aimInfo, status);

        status = EG_getBodyTopos(copy_body_tess[bodyIndex], NULL, FACE, &numFace, &faces);
        AIM_STATUS(aimInfo, status);
    ...
    for (iface = 0; iface < numModelFace; iface++) {
      ...
      if (status == EGADS_NOTFOUND) {
        ...
        // If AFLR_GBC is not set, and viscous BC is needed, set it
...
//    }
    ...
    for (bodyIndex = 0; bodyIndex < ptr->ntess; bodyIndex++) {
        ...
        status = EG_getBodyTopos(ptr->bodies[bodyIndex], NULL, FACE, &nface, NULL);
        AIM_STATUS(aimInfo, status);
        ...
        for (i = 0; i < nface; i++, iface++) {
            ...
            status = EG_attributeAdd(ptr->tess[bodyIndex], attrname, ATTRINT,
        ...
        remove(aimFile);
        status = EG_saveTess(ptr->tess[bodyIndex], aimFile);
        AIM_STATUS(aimInfo, status);
    }
