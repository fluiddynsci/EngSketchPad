...
enum aimInputs
{
  ...
  inSurface_Mesh,
  inDesign_Variable,
  NUMINPUT = inDesign_Variable   /* Total number of inputs */
};
              ...
              massProperties<T>& prop)
{
  ...
  const meshStruct *nasMesh = &masstranInstance->feaProblem.feaMesh;
  ...
  for (i = 0; i < nasMesh->numElement; i++) {
    ...
    if (nasMesh->element[i].elementType == Node ) {
      ...
      if (feaData != NULL && feaData->elementSubType == ConcentratedMassElement) {
        ...
        xcent = node[elem[0]].xyz[0] * Lscale + feaProperty->massOffset[0];
        ycent = node[elem[0]].xyz[1] * Lscale + feaProperty->massOffset[1];
        zcent = node[elem[0]].xyz[2] * Lscale + feaProperty->massOffset[2];
                 ...
                 /*@unused@*/ int restart, /*@unused@*/ capsValue *inputs)
{
  ...
  if (inputs[inDesign_Variable-1].nullVal == NotNull) {
    ...
    for (i = 0; i < masstranInstance->feaProblem.numDesignVariable; i++) {
      ...
      name = masstranInstance->feaProblem.feaDesignVariable[i].name;
    ...
    for (ibody = 0; ibody < masstranInstance->numMesh; ibody++) {
      for (i = 0; i < masstranInstance->feaMesh[ibody].numNode; i++, j++) {
        sensNodes[j].xyz[0] = masstranInstance->feaMesh[ibody].node[j].xyz[0];
        sensNodes[j].xyz[1] = masstranInstance->feaMesh[ibody].node[j].xyz[1];
        sensNodes[j].xyz[2] = masstranInstance->feaMesh[ibody].node[j].xyz[2];
      }
