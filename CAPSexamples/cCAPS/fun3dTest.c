...
int main(int argc, char *argv[])
{
    ...
    // CAPS objects
    capsObj          problemObj, fun3dObj, tempObj;
    capsErrs         *errors;
    capsOwn          current;
    ...
    printf("\n\nAttention: fun3dTest is hard coded to look for ../csmData/cfdMultiBody.csm\n");
    ...
    if (argc > 2) {
        ...
        printf(" usage: fun3dTest analysisDirectoryPath!\n");
        return 1;
    ...
    status = caps_open("../csmData/cfdMultiBody.csm", "FUN3D_Example", &problemObj);
    if (status != CAPS_SUCCESS) goto cleanup;

    status = caps_info(problemObj, &name, &type, &subtype, &link, &parent, &current);

    // Now load the fun3dAIM
    status = caps_load(problemObj, "fun3dAIM", analysisPath, NULL, NULL, 0, NULL, &fun3dObj);
    if (status != CAPS_SUCCESS) goto cleanup;
